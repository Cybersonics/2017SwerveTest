...
public class Pixy2 {
	...
	public Pixy2(I2CDevice i2c) {
		//this.i2cPort = (byte) i2cPort.value;
		//this.i2cAddress = i2cAddress;
		//I2CJNI.i2CInitialize((byte) i2cPort.value);
		this.i2c = i2c;
	...
	private int getByte() {
		...
		while (!i2c.readRaw(buffer, 1)) sleep(20);
	...
	private boolean getStart() {
		...
		while (true) {
			...
			if (w == 0 && lastw == 0) {
				...
				getByte();
				System.out.println("reorder " + hex.toString());
			}
			lastw = w;
		}
	}
	...
	private List<Block> getBlocks() {
		...
		for (int blockCount = 0; blockCount < MAX_BLOCKS;) {
			...
			if (checksum == sum) {
				...
				System.out.println("checksum error (" + sum + " != " + checksum + ") " + hex.toString());
			}
